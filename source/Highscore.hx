...
class Highscore
{
	...
	public static var songFC:Map<String, String> = new Map();
	#else
	public static var weekScores:Map<String, Int> = new Map();
	...
	public static function saveScore(song:String, score:Int = 0, ?diff:Int = 0, ?rating:Float = -1,?fc:String = ''):Void
	{
		var daSong:String = formatSong(song, diff);
		...
		trace('fc ' + fc + ', saveFC ' + (songFC.exists(daSong) ? songFC.get(daSong) : ''));

		if (songFC.exists(daSong) && fc != '') {
			if (songFC.get(daSong) != 'pfc')
				setFC(daSong,fc);

		}
		else {
			if (fc != '')  setFC(daSong,fc);
		}
